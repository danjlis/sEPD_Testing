...
void Analyze(std::vector<std::string> *filenames, const std::string data_dir, const std::string save_dir_raw, const std::string save_dir_plot, const std::string save_dir_root, int ch1, int ch2){
  bool debug = true;
  ...
  for (int i = 0; i < size; i++){
    ...
    std::string fname;
    fname = MakeRootFile(filenames->at(i), data_dir, save_dir_raw, n_channels);
    if (debug){
      cout<<"Made Root File"<<endl;
    }
    std::string root_path= save_dir_root + fname + ".root";
    ...
    drawText(Form("Channels %s to %s",ch_1, ch_2),xPos,yPos-2*dy2,0, 1, fontSize, fontType);
    //drawText("SiPM Switched",xPos,yPos-3*dy2,0, 1, fontSize, fontType);

    TString plot_name = Form(,ch_1, ch_2);
    c_full->SaveAs(Form("%s%s.png", save_dir_plot, fname));
    c_full->SaveAs(Form("%s%s.pdf", save_dir_plot, fname));

    TFile *out_hist_file = new TFile(Form("%s%s_hists.root", save_dir_root, fname ), "recreate");
    for (int i = 0; i < NTILE; i++){
      int ns = 0;
      int oo = 0;
      for (std::vector<int>::iterator it = channels.begin(); it != channels.end(); ++it){
...
// Input configuration file
  ...
  GetFileName(filenames, sector, all_runs, ch1, ch2);

  // Now we have out file names

  // analyze the data
  Analyze(filenames, data_dir, save_dir_raw, save_dir_plot, save_dir_root, ch1, ch2);
