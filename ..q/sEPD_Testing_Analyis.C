...
void Full_Scan_Test(TString fname = "../data/sEPD_20220318-1637_sector1_Full_ch8.txt", int sector = 1, bool makeRootFile = true){
  ...
  char *tlist = new char[100];

  sprintf(tlist, "%d-%d%d-%d%d%d",1900+ltm->tm_year,1+ ltm->tm_mon, ltm->tm_mday, 5+ltm->tm_hour, 30+ltm->tm_min, ltm->tm_sec);
  TString cap = Form("_%s", tlist);
  make_sEPD_rootfile_v3(fname, cap, ttt);
//  char *date = new char[50];
//  char *test = new char[50];
//  char *addon = new char[50];
//  int sector;
  ...
  /////////////////////////////////////
  // START DRAWING
  TString savedir = "s01_ch8"; //Form("s%d_ch_8", sector);
  float xPos = 0.12;
  //float xPos = 0.85;
  float yPos = 0.86;
  float yPos2 = 0.2;
